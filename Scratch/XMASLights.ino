...
SimplePatternList gPatterns = { /*randomRainbow, randomRainbow, */ confetti,
		juggle, confetti,
		solid, solid, confetti,
		rainbow, rainbowWithGlitter, confetti, /*, bpm */ };

uint8_t gCurrentPatternNumber = 0; // Index number of which pattern is current
uint8_t gHue = 0; // rotating "base color" used by many of the patterns
static bool ledBaseNeedsInit = true;
...
static int ledBase[NUM_LEDS];
void randomRainbow()
{
	if (ledBaseNeedsInit) {
		for (int i = 0; i < NUM_LEDS; i++) {
			ledBase[i] = random16(255);
		}
		ledBaseNeedsInit = false;
	}

	for (int i = 0; i < NUM_LEDS; i++) {
		leds[i] = CHSV( (ledBase[i] + gHue) % 256, 200, 255);
	}
}
